HHTDatePicker = HControl.extend
  componentName: 'hht_datepicker'
  markupElemNames: [ 'bg', 'input_parent', 'weekday', 'value', 'icon' ]
  ...
  refreshValue: ->
    ...
    ELEM.setHTML( @markupElemIds.weekday, @moment( @value * 1000 ).format( @options.weekdayFormat ) )
    true
  ...
  die: ->
    ...
    Event.stopObserving( @markupElemIds.weekday, 'click', @_iconClickFn )
  ...
  openCalendar: ->
    ...
    @_calendar = HHTMiniCalendar.new( [ x + ( w - 250 ) / 2, y + 34, 250, 250 ], @app,
      value: @value
      target: @
    )
